--[[
    ...
    Decursive (v 2.5.2) add-on for World of Warcraft UI
...
function D:BetaWarning()
    ...
    if (("2.5.2"):lower()):find("beta") or ("2.5.2"):find("RC") or ("2.5.2"):find("Candidate") or alpha then
        ...
        if D.VersionTimeStamp ~= 0 then
            ...
            if time() > D.VersionTimeStamp + VersionLifeTime then
                ...
                if time() - self.db.global.LastExpirationAlert > 48 * 3600  then
                    StaticPopup_Show ("Decursive_Notice_Frame", "|cff00ff00Decursive version: 2.5.2|r\n\n" .. "|cFFFFAA66" .. L["DEV_VERSION_EXPIRED"] .. "|r");
        ...
        if self.db.global.NonRealease ~= "2.5.2" then
            self.db.global.NonRealease = "2.5.2";
...
function D:GetSpellsTranslations(FromDIAG)
    ...
    Spells = {
		...
		["SPELL_ALLURING_SONG"]			= {		81464,									 }, -- Bards
...
T._LoadedFiles["DCR_init.lua"] = "2.5.2";
